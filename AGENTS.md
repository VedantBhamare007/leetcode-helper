...
Owner: Vedant Bhamare  
GitHub: https://github.com/VedantBhamare007
...
If the original prompt provides a C++ signature, preserve it exactly. Prefer the
style already shown by the prompt.
...
- Do not include `#include` statements or `using namespace std;` unless the user asks.
...
- `vector<int> twoSum(...)`: Defines the method with the input vector and target value.
- `unordered_map<int, int> seen;`: Stores each number we have visited and its index.
- `for (int i = 0; i < nums.size(); i++)`: Loops through each number while tracking its index.
...
- Negative numbers: handled because subtraction and hash map lookup work the same way.
